...
module.exports = function(O){
  ...
  if (S.settings.has_ws){
    ...
    S.ws.use(function(socket, next){
      ...
      , function(cb){
          return S.sessionsStore.get(Belt.get(socket, 'handshake.secureCookies.' + S.settings.http.sessions.key)
                 ...
                 , Belt.cs(cb, gb, 'session', 1));
        }
      ], function(err){
        socket.$session = gb.session;
        return next(err);
      });
    });
        ...
        return socket.on(r.event, function(d){
          ...
          return Async.waterfall([
            function(cb){
              ...
              return S.sessionsStore.get(gb.sid, Belt.cs(cb, socket, '$session', 1));
            }
          ], function(err){
