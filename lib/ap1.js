...
module.exports = function(O){
  ...
  if (S.settings.has_ws){
    ...
    S['ws'] = new IO(S.settings.ws);

    S.ws.use(function(socket, next) {
      var req = socket.handshake;
      var res = {};
      return S.cookieParser(req, res, function(err) {
        if (err) return next(err);
        return S.session(req, res, next);
      });
    });
    ...
    S.ws['addRoute'] = function(route, method, options){
      ...
      if (o.middleware) return S.ws.use( function(socket, next){
        return method.call(S, S.ws.$transaction(route, {}, socket, next));
      });

      o.method = function(data){
        return method.call(S, S.ws.$transaction(route, data, this, undefined));
      };
...
console.log(socket.handshake);
      return _.each(this.$routes, function(r){
