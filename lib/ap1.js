...
module.exports = function(O){
  ...
  if (S.settings.has_socket){
    ...
    S.socket['$transaction'] = function(args, socket, cb){
      var d = args
        , t = {
            '$request': socket
          , '$response': socket
    ...
    S.socket.on('connection', function(socket){
      return socket.on('data', function(_data){
        var data = Belt.parse(_data.toString()); //JSON required
        if (!data) return;
        ...
        return Async.waterfall([
          ...
          return S.socket.$transaction(data, socket, function(d){
            return rt.method.call(socket, d);
          });
        });
      });
    });
