...
, 'add basic http route': function(test){
    ...
    return Request({
      'url': 'https://localhost:' + O.http.port + '/route'
    , 'jar': gb.jar
    , 'json': true
    , 'strictSSL': false
...
, 'add basic http middleware': function(test){
    ...
    return Async.waterfall([
      function(cb){
        return Request({
          'url': 'https://localhost:' + O.http.port + '/authorized?unauthorized=true'
        , 'jar': gb.jar
        , 'json': false
        , 'strictSSL': false
        }, function(err, res, body){
    ...
    , function(cb){
        return Request({
          'url': 'https://localhost:' + O.http.port + '/authorized'
        , 'jar': gb.jar
        , 'json': true
        , 'strictSSL': false
        }, function(err, res, body){
...
, 'inspect http transaction object': function(test){
    ...
    return Request({
      'url': 'https://localhost:' + O.http.port + '/1/2/transaction.json?good=bye&hi=there'
    , 'jar': gb.jar
    , 'json': true
    , 'method': 'POST'
    , 'strictSSL': false
...
, 'http session data': function(test){
    ...
    return Async.waterfall([
      function(cb){
        return Request({
          'url': 'https://localhost:' + O.http.port + '/session/set.json?visited=true'
        , 'jar': gb.jar
        , 'json': true
        , 'method': 'post'
        , 'body': {foo: 'bar'}
    ...
    , function(cb){
        return Request({
          'url': 'https://localhost:' + O.http.port + '/1/2/transaction.json?'
        , 'jar': gb.jar
        , 'json': true
        , 'method': 'POST'
        , 'body': {'foo': 'bar'}
...
, 'server-side socket.io route': function(test){
    ...
    gb.sio = new IO('https://localhost:' + gb.api.settings.http.port);
...
, 'connect to ws server': function(test){
    ...
    gb.ws = new WS('wss://localhost:' + gb.api.settings.ws.port);
